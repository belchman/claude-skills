...
description: Stress-test a plan against the project's domain language (CONTEXT.md) and recorded decisions (docs/adr/), sharpening terminology and capturing crystallized decisions inline. Use when the user wants to interrogate a plan that lives in this codebase — has a PRD, design sketch, or feature idea grounded in real domain concepts. Pairs with grill-me (which does general ideation without project grounding).
---
