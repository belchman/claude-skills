...

{
  echo "# $PROJECT_NAME"
  echo ""
  if [ -n "$PROJECT_DESC" ]; then echo "$PROJECT_DESC"; echo ""; fi
  if [ -n "$TECH_STACK" ]; then
    echo "## Tech Stack"; echo ""; echo "$TECH_STACK"; echo ""
  fi
  if [ -n "$STRUCTURE" ]; then
    echo "## Project Structure"; echo ""; echo '```'; echo "$STRUCTURE"; echo '```'; echo ""
  fi
  echo "## Commands"; echo ""
  [ -n "$INSTALL_CMD" ]      && echo "- Install: \`$INSTALL_CMD\`"
  [ -n "$BUILD_CMD" ]        && echo "- Dev/Build: \`$BUILD_CMD\`"
  [ -n "$TEST_CMD" ]         && echo "- Test: \`$TEST_CMD\`"
  [ -n "$SINGLE_TEST_CMD" ]  && echo "- Single test: \`$SINGLE_TEST_CMD\`"
  [ -n "$LINT_CMD" ]         && echo "- Lint: \`$LINT_CMD\`"
  [ -n "$FORMAT_CMD" ]       && echo "- Format: \`$FORMAT_CMD\`"
  echo ""
  if [ -n "$STYLE_RULES" ] || [ -n "$COMMIT_STYLE" ]; then
    echo "## Code Style"; echo ""
    [ -n "$COMMIT_STYLE" ] && echo "- Commit messages: $COMMIT_STYLE"
