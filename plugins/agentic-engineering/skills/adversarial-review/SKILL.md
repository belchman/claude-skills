...
- **State-machine completeness**: only fires when the code has BOTH (a) a finite set of state values AND (b) a dispatcher that branches on those values — a `case "$state" in …`, a `switch`/`match` over the enum, or a long `if/elif` chain on the field. An enum used only for display, logging, or equality comparison (no dispatcher) is NOT an FSM and should be left alone. When both conditions hold: enumerate the state values from the code/spec and verify every reachable state has a handler arm AND a corresponding test. Missing arms appear as `*) echo "unknown state" ;;` catchalls or unmatched `default:` clauses that hide real bugs. Look for FSM ground truth in: a `case`/`switch`/`match` body, `last_completed_step` / `status` / `phase` / `state` / `step` field names that get dispatched on (not just read or rendered), lists of constants in `### States` or `## State machine` sections of a spec, a state-transition diagram (Mermaid `stateDiagram`, ASCII boxes).
