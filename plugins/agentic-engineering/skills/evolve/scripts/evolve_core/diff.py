...
# Cap inputs to keep regex backtracking bounded and to refuse absurd payloads.
MAX_DIFF_RESPONSE_BYTES = 8 * 1024 * 1024  # 8 MB


def _check_size(value: str, label: str) -> None:
    if len(value.encode("utf-8", errors="ignore")) > MAX_DIFF_RESPONSE_BYTES:
        raise ValueError(f"{label} exceeds {MAX_DIFF_RESPONSE_BYTES} bytes; refusing to parse.")
...
def extract_diffs(
    ...
    _check_size(llm_response, "LLM response")
...
def apply_diff(
    ...
    _check_size(original_code, "Original code")
