...
def custom_sampler_runtime_config(
    ...
    # The custom sampler is `exec`'d as Python — confine it to the workspace
    # so an attacker-controlled spec cannot point at /tmp/evil.py or a system
    # file. We also refuse to leak the workspace into sys.path (was a
    # search-path hijack vector during exec_module).
    if workspace_root is not None:
        workspace_root_abs = Path(workspace_root).resolve()
        try:
            resolved_path.relative_to(workspace_root_abs)
        except ValueError as exc:
            raise PermissionError(
                f"Custom sampler must live inside the workspace ({workspace_root_abs}); "
                f"got: {resolved_path}"
            ) from exc

    runtime_config: Dict[str, Any] = {
        "custom_sampler_path": str(resolved_path),
        # Only the sampler's own directory — no workspace root, no extra paths.
        "custom_sampler_search_paths": [str(resolved_path.parent)],
    }
    if custom_class:
...
def validate_custom_sampler_for_workspace(
    ...
    # NEVER exec a sampler module just because someone wrote it into the spec.
    # `exec_module` runs arbitrary Python; deferring it until approval.confirmed
    # forces a human to acknowledge "this is code we will run".
    if not bool(spec.get("approval", {}).get("confirmed", False)):
        # Static checks only: path resolution + existence + scope. Do NOT load
        # the module yet.
        try:
            runtime_config = custom_sampler_runtime_config(spec, workspace_root)
        except PermissionError as exc:
            return str(exc)
        path = Path(runtime_config.get("custom_sampler_path", custom_path))
        if not path.exists():
            return f"Custom sampler path does not exist: {path}"
        return ""
