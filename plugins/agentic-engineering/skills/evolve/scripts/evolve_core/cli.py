...
# Caps to defend against runaway / hostile inputs.
MAX_RESULTS_JSON_BYTES = 64 * 1024 * 1024      # 64 MB
MAX_SEED_FILE_BYTES = 16 * 1024 * 1024         # 16 MB
MAX_INPUT_FILE_BYTES = 64 * 1024 * 1024        # 64 MB (for read/write/copy)


def _check_file_size(path: Path, limit: int, label: str) -> None:
    try:
    ...
    if size > limit:
        raise SystemExit(
            f"{label} exceeds {limit} bytes ({size} bytes at {path}); refusing to load."
        )


def _safe_open_for_read(path: Path):
    """Open without following symlinks.

    Defends against TOCTOU swaps where an attacker who can write into the
    allowed scope replaces a regular file with a symlink to /etc/shadow between
    the path check and the open.
    """
...
def _safe_copy(src: Path, dst: Path) -> None:
    """Copy src→dst with O_NOFOLLOW on both ends and a size cap."""
    _check_file_size(src, MAX_INPUT_FILE_BYTES, "Source file")
    with _safe_open_for_read(src) as handle:
        data = handle.read()
    # Refuse to clobber an existing symlink at dst.
    if dst.is_symlink():
        raise SystemExit(f"Refusing to copy through symlink at destination: {dst}")
    fd = os.open(str(dst), os.O_WRONLY | os.O_CREAT | os.O_TRUNC | os.O_NOFOLLOW, 0o600)
    try:
        os.write(fd, data)
    finally:
        os.close(fd)
...
def extract_seed_items(markdown_path: Path) -> List[CognitionItem]:
    ...
    _check_file_size(markdown_path, MAX_SEED_FILE_BYTES, "Cognition seed file")
...
def cmd_eval_run(args: argparse.Namespace) -> int:
    ...
    if source_code != step_code_path:
        _safe_copy(source_code, step_code_path)
    ...
    # Substitute the {placeholders} BEFORE shlex.split, then parse as argv. The
    # legacy `quoted_*` variants only added literal double-quotes (which shell
    # would consume); shlex.split handles them as a quoted token. We refuse to
    # run with shell=True — that path was RCE if the spec was untrusted.
    ...
    # Use shlex-quoted variants so substitution is safe even when values
    # contain spaces / metacharacters. Templates relying on the old `quoted_*`
    # form still work because we keep those, but they now contain a properly
    # escaped value rather than a bare "$value" double-quoted blob.
    safe_context = {
        **context,
        **{f"quoted_{key}": shlex.quote(value) for key, value in context.items() if not key.startswith("quoted_")},
    }
    formatted = command.format(**safe_context)
    try:
        argv = shlex.split(formatted)
    except ValueError as exc:
        raise SystemExit(f"Evaluation command could not be parsed: {exc}") from exc
    if not argv:
        raise SystemExit("Evaluation command is empty after substitution.")
    ...
    if results_path.exists():
        _check_file_size(results_path, MAX_RESULTS_JSON_BYTES, "Evaluator results.json")
...
def cmd_db_record(args: argparse.Namespace) -> int:
    ...
    if args.results_file:
        ...
        if results_path.exists():
            ...
            _check_file_size(results_path, MAX_RESULTS_JSON_BYTES, "Results file")
            results = json.loads(results_path.read_text(encoding="utf-8"))
            if results_path != step_dir / "results.json":
                _safe_copy(results_path, step_dir / "results.json")
    ...
    if args.analysis_file:
        ...
        _check_file_size(analysis_path, MAX_INPUT_FILE_BYTES, "Analysis file")
    ...
    if best_updated:
        ...
        _safe_copy(step_code_path, run_best_dir / "code")
        results_copy = step_dir / "results.json"
        if results_copy.exists():
            _safe_copy(results_copy, run_best_dir / "results.json")
    append_round_log(
        run_dir,
        "db_record",
...
def cmd_files_read(args: argparse.Namespace) -> int:
    ...
    _check_file_size(target, MAX_INPUT_FILE_BYTES, "Read target")
    with _safe_open_for_read(target) as handle:
        content = handle.read().decode("utf-8")
    return emit_json({"content": content, "path": str(target)})
...
def cmd_files_write(args: argparse.Namespace) -> int:
    ...
    if args.from_file:
        ...
        _check_file_size(content_source, MAX_INPUT_FILE_BYTES, "Write source")
        with _safe_open_for_read(content_source) as handle:
            content = handle.read().decode("utf-8")
    else:
        content = args.content or ""

    target.parent.mkdir(parents=True, exist_ok=True)
    # Refuse to write through a symlink at the destination — prevents TOCTOU
    # symlink-swap to a sensitive path inside writable scope.
...
def cmd_files_diff(args: argparse.Namespace) -> int:
    ...
    _check_file_size(left, MAX_INPUT_FILE_BYTES, "Diff left")
    _check_file_size(right, MAX_INPUT_FILE_BYTES, "Diff right")

    with _safe_open_for_read(left) as handle:
        left_text = handle.read().decode("utf-8")
    with _safe_open_for_read(right) as handle:
        right_text = handle.read().decode("utf-8")
    diff = "".join(
        unified_diff(
