...
# Cap structured spec/JSON files. A spec is meant to be ~kilobytes; anything
# larger is either a bug or an attempt to OOM the orchestrator.
MAX_STRUCTURED_FILE_BYTES = 4 * 1024 * 1024  # 4 MB
...
def normalize_spec_path(workspace_root: Path, raw_path: str) -> str:
    ...
    if not path.is_absolute():
        # Reject relative paths that escape the workspace via `..`.
        resolved = (workspace_root / path).resolve()
        try:
            resolved.relative_to(workspace_root)
        except ValueError as exc:
            raise PermissionError(
                f"Spec path escapes the workspace: {raw_path}"
            ) from exc
    ...
    try:
        ...
        # Absolute path outside the workspace — refuse rather than silently
        # keeping it (the old behavior allowed `/etc/shadow` style targets).
        raise PermissionError(
            f"Absolute spec path is outside the workspace ({workspace_root}): {resolved}"
        ) from exc
...
def ensure_path_allowed(run_dir: Path, target_path: Path) -> Path:
    ...
    for raw_path in allowed_paths:
        resolved = resolve_path(workspace_root, raw_path)
        # Refuse writable_paths that escape the workspace. The spec is
        # attacker-controllable when imported from a file; without this clamp,
        # `writable_paths: ["/"]` granted full-disk write.
        try:
            resolved.relative_to(workspace_root)
        except ValueError:
            continue
...
def load_structured_file(path: Path) -> Dict[str, Any]:
    ...
    if size > MAX_STRUCTURED_FILE_BYTES:
        raise ValueError(
            f"Spec/JSON file exceeds {MAX_STRUCTURED_FILE_BYTES} bytes "
            f"({size} bytes at {path}); refusing to load."
        )
