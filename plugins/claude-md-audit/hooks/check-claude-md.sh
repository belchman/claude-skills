#!/usr/bin/env bash
# PreToolUse hook for Edit/Write. Runs lint-claude-md.sh against any target
# file whose basename is CLAUDE.md, and blocks the write if the lint reports
# errors (exit 2 from the linter).
#
...
# (e.g. plugins/foo/CLAUDE.md) are also checked.
...
# Capture linter output and exit code. `if ! cmd` would negate $?, so use the
# `&&/||` form to preserve the underlying status.
output="$(bash "$linter" "$file_path" 2>&1)" && rc=0 || rc=$?

# Exit code 2 from the linter == errors. Anything else (1 = warnings, 0 = pass)
# we let through so the user isn't blocked on style nits.
if [ "$rc" -eq 2 ]; then
  {
    echo "claude-md-audit: lint reported errors in $file_path"
    echo
    printf '%s\n' "$output"
    echo
